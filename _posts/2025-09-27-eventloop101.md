...
Vamos testar duas abordagens do mesmo algoritmo, que agrupa strings de um arquivo de acordo com letras do alfabeto e expõe o resultado via endpoint HTTP: uma versão sem Worker Threads e outra com Worker Threads. Exemplo:
...
Versão sem Worker Threads (bloqueia a call stack; durante a execução, tente fazer um `GET /outra-api`).
...
Versão com Worker Threads (não bloqueia a call stack; durante a execução, tente fazer um `GET /outra-api`).
